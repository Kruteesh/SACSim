...
class Solver:
    def __init__(self,i_model,list_particles,R_sun,h_list,phi_0):
    ...
    def L_dec(self):
        return Lambdas.inverse_lengths(self.i_model,self.list_particles).inv_L_dec()


    def L_inte(self):
        return Lambdas.inverse_lengths(self.i_model,self.list_particles).inv_L_inte()#####change A for sun. !!!!!!
    ...
    def solver(self,r_ratio,rho_data):
        mceq_run.set_interaction_model(self.i_model)
        print("S"+str(mceq_run.pman[2212].cs[-1]))
        sol_odeint={}
        C=self.C()
        D=self.D()
        L_inte=self.L_inte()
        ...
        for h in self.H():
            ...
            sol_odeint[h]=odeint(RHS_ivp,
                self.phi,X[h],args=(C,D,L_inte,L_dec,x2rho[h]),tfirst=True)#,[X[h][0],X[h][-1]],vectorized=True,
