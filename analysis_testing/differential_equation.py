...
class Solver:
    ...
    def X2R(self,r_ratio,rho_data):
        X2r={}
        for h in self.H():
            print(self.X_rho(r_ratio,rho_data)[h].shape,np.append(-r_ratio[:-1],r_ratio[::-1][3:]).shape)
            X2r[h]=UnivariateSpline(self.X_rho(r_ratio,rho_data)[h],
                                np.append(-r_ratio[:-1],r_ratio[::-1][4:])
                                          ,k=1,s=0)
        return X2r
    ...
    def solver(self,r_ratio,rho_data):
        ...
        L_inte=self.L_inte()
        L_dec=self.L_dec()
        X=self.X_rho(r_ratio,rho_data)[0]
        x2rho=self.X_rho(r_ratio,rho_data)[1]
        ...
        for h in self.H():
            ...
            sol_ivp[h]=solve_ivp(
                                    RHS_ivp,
                                  [X[h][0],X[h][-1]],
                                self.phi,method='LSODA',args=(C,D,L_inte,L_dec,x2rho[h]))#vectorized=True,
